...
def extract_feature(X, sample_rate, **kwargs):
    ...
    result = np.array([])
        # Resample
        #Y_16K = librosa.resample(X,sample_rate)
        # Trimm Start/End
        #trimmed, index = librosa.effects.trim(Y_16K,top_db=20,frame_length=512,hop_length=64)
...
pickle.dump(scaler, open('audio_utils/scaler_speaker_aug.bin','wb'))
...
if not os.path.isdir("audio_utils"):
    os.mkdir("audio_utils")
pickle.dump(clf, open("audio_utils/classifier_speaker_OvR_aug.model", "wb"))
